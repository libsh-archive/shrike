 ...
 */
class
SH_DLLEXPORT ShGlyph : public ShRefCountable {
public:
  ShGlyph(); ///< Construct an empty image
  //ShGlyph(int width, int height, int depth); ///< Construct a black
                                             ///image at the given width/height/depth
  //ShGlyph(const ShGlyph& other); ///< Copy an image
  ...
  float halfy() const; ///< shift coordinates in y axis
                        ///the image
  void ShGlyph::loadGlyph(const std::string&);
