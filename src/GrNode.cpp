...
GrNode::GrNode(const std::string& label,
    ...
    m_face(face)
{
  ...
  m_width = 0;
  m_height = 0;
  m_font_height = m_face->height()/2.0;
...
void GrNode::draw_box()
{
  ...
  glBegin(GL_QUADS); {
    ...
    glVertex3f(m_x + m_width + shadow, m_y + m_height - shadow, 0.0);
    glVertex3f(m_x + shadow, m_y + m_height - shadow, 0.0);
  } glEnd();
