...
public:
  ...
  return_type operator[](const ShTexCoord2f tc) const 
  {
    ...
    interp1 = mad(ftc1(1), (*bt)[ShAttrib2f(u(0),umin(1))], interp1);
    // the second
    return_type interp2 = ftc2(1) * (*bt)[ShAttrib2f(umin(0),u(1))];
    interp2 = mad(ftc1(1), (*bt)[umin], interp2);
    // both interpolations are returned
...
/** define the interpolation of data with the use of cardinal splines
  */
template<typename T>
class CardinalSplineInterp : public T, public ShMemoryDep {
public:
  typedef T parent_type;
  typedef typename T::return_type return_type;  
  typedef typename T::base_type base_type;
...
/** define the interpolation of data with the use of smooth spline
  ...
  */
template<typename T>
class SmoothSplineInterp : public T, public ShMemoryDep {
public:
  typedef T parent_type;
  typedef typename T::return_type return_type;  
  typedef typename T::base_type base_type;
  ...
  void memory_update() 
  {
    ...
    float* newdata = (float*)newmem->hostStorage()->data();
    m_lambda /= -100.0;
    float zp = 1.0 + 0.5/m_lambda - 0.5*sqrt(1.0+4*m_lambda)/m_lambda;
    int k0 = 10;
    float c0 = -1.0/m_lambda;
    for (int e = 0; e < stride; e++) {
    // clear the data
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
