 ...
 *
 *  Integrates the use of cube-map which are accessed
 *  with a 3D vector
 *
 */
template<typename T>
...
public:
  ...
  return_type operator()(const ShAttrib3f u) const 
  {
    ...
    ShAttrib1f maxcoord = SH::max(SH::max(absv(0), absv(1)), absv(2)); // find the max coordinates = the direction
    ShAttrib3f textcoord = -v / maxcoord; // scale by the direction coordinate
    ...
    ShAttrib2f tc = cond(abs(Xface), ShAttrib2f(textcoord(2),textcoord(1)) ,
			 cond(abs(Yface), ShAttrib2f(textcoord(0), textcoord(2)),
			      ShAttrib2f(textcoord(0), textcoord(1))));
    ...
    tc(0) = cond(Xface+Yface+Zface, tc(0), tc(0)+0.5); // if the direction coordinate is positive or negative
