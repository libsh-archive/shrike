...
protected:
  ...
  ShHostMemoryPtr Reduction(float *data, int width, int height, int stride, int sigma) 
  {
    ...
    for(int i=0 ; i<width ; i++) {
      for(int j=0 ; j<height ; j++) {
	for(int k=0 ; k<stride ; k++) {
	  reducdata[(j*width+i)*stride+k] = 0.25*(data[(2*j*doublewidth+2*i)*stride+k] + 
						  data[((2*j+1)*doublewidth+2*i)*stride+k] +
						  data[(2*j*doublewidth+2*i+1)*stride+k] + 
						  data[((2*j+1)*doublewidth+2*i+1)*stride+k]);
	}
      }
    }
#if (USE_GAUSSIAN)
